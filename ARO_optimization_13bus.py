...
REV 21 10 2024
SCRIPT MADE FOR RESULTS RETRIEVAL AND OVERCURRENT RELAY PARAMETERIZATION CURVES - IEEE 13 BUS'''
...
'''MAIN FUNCTION'''
def main():
    system_13_bus = (r"models_and_results/ieee_13bus/IEEE13Mod.dss", 'IEEE13bus') 
    simulated_sys = system_13_bus # Select the system to be analyzed
...
def get_dss(path): 
    ...
    while True: 
        if 'Capacitor' not in dss.pdelements._name_read() and 'sourcebus' not in dss.cktelement._bus_names()[0]:
...
def calc_tms(relay, pairs, k_values, cti, min_tms):
    ...
    axins = inset_axes(plt.gca(), width="50%", height="40%", loc='center right')
    ...
    for i in range(3): winsound.Beep((i + 1) * 1000, 1500) # make a alert sound when the optimization is finished
    tms_list = solution_history[np.argmin(list_best_solution)]  # get the solution with minimum fitness as tms_list
    return tms_list
...
def execute_iteration(problem_constrained, k_close_fault, k_pairs, min_tms, nr, pairs, cti, iter):
    ...
    # Inicializando as variáveis globais para a iteração
    k_close_fault_g = k_close_fault
    k_pairs_g = k_pairs
    min_tms_g = min_tms
    nr_g = nr
