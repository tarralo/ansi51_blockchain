...
REV 21 10 2024
SCRIPT MADE FOR RESULTS RETRIEVAL AND OVERCURRENT RELAY PARAMETERIZATION CURVES - IEEE 37 BUS'''
...
'''MAIN FUNCTION'''
def main():
    system_37_bus = (r"models_and_results/ieee_37bus/ieee37_SC_Currents.dss", 'IEEE37bus')
    simulated_sys = system_37_bus # Select the system to be analyzed
...
def get_dss(path): 
    ...
    while True: 
        if 'Capacitor' not in dss.pdelements._name_read() and 'jumper' not in dss.pdelements._name_read() and 'reg' not in dss.pdelements._name_read() and 'sourcebus' not in dss.cktelement._bus_names()[0]:
    ...
    for phases in range(1, 4): # 1, 2 and 3 phase faults
        ...
        for bus in fault_df.columns[1:]: 
            ...
            if phases == 1: # 1 phase fault
                ...
                dss.text(r'Solve mode=direct') # Solve the system
...
'''CALCULATE THE K'S VALUES - TEST THE USE OF FAULT 3PH (18/10 16:38)'''
def calculate_k(nb, nr, fault, relay, a, b): 
    ...
    k_values = fault[2].copy(deep=True) # create a dataframe to store the k values
    ...
    for i in range(nr): # in range of rows = number of relays
        for j in range(nb): # in range of columns = number of buses
            if fault[2].iloc[i, j + 1] != 0:  # if the fault current is different from 0
                isc = fault[2].iloc[i, j + 1] # get the fault current
...
def calc_tms(relay, pairs, k_values, cti, min_tms):
    ...
    # the bounds of all relays are the min_tms and 0.05 
    ...
    repetition = 12 # number of repetitions
    iter = 0
    while iter < 1: 
        start = time.time()
        list_best_solution = []
...
def execute_iteration(problem_constrained, k_close_fault, k_pairs, min_tms, nr, pairs, cti, iter):
    ...
    if iter == 0: model = ARO.OriginalARO(epoch=2000, pop_size=1000)
